...
public class GridHotkeyMaker {
    ...
    private static boolean hasColorCodes(String xx) {
        String lowerCase = xx.toLowerCase();
        int colorIndex = lowerCase.indexOf("|c");
        if (colorIndex == -1) {
            return false;
        }
        ...
        for (int i = 0; i < 8; i++) {
            char c = lowerCase.charAt(i + colorIndex + 2);
    ...
    public static void main(final String[] args) {
        ...
        String[][] grid = {{"Q", "W", "E", "R"}, {"A", "S", "D", "F"}, {"Z", "X", "C", "V"}};
...
//        System.exit(0);
        for (final War3ID unitId : objectData.getUnits().keySet()) {
            ...
            boolean[][] sharedCommandCardButtons = new boolean[3][4];
            for (UILookupField menuMakingKey : MENU_MAKING_KEYS) {
                String trainsList = unitType.getFieldAsString(menuMakingKey.getFieldMetaKey(), 0);
                String[] trainsListItems = trainsList.split(",");
                if(trainsList.length()==4) {
                    trainsListItems = new String[]{trainsList};
                }
                boolean[][] usedButtonPositions = menuMakingKey.isSharedCommandCard() ? sharedCommandCardButtons : new boolean[3][4];
                ...
                for (String key : trainsListItems) {
                    ...
                    War3ID trainedUnitId = War3ID.fromString(key);
                    MutableObjectData dataByType = objectData.getDataByType(menuMakingKey.getDataType());
                    if (dataByType == null) {
                        throw new IllegalStateException(menuMakingKey.getDataType() + " is null");
                    }
                    MutableGameObject trainedUnit = dataByType.get(trainedUnitId);
                    if (trainedUnit == null) {
                        System.err.println(trainedUnitId);
                    } else {
                        if (unitId.toString().equals("h013")) {
                            System.err.println("Assigning " + trainedUnit.getFieldAsString(menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.UPGRADES ? War3ID.fromString("gnam") : menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.ABILITIES ? War3ID.fromString("anam") : War3ID.fromString("unam"), 0));
                        }
                        if(menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.ABILITIES ) {
                            String abilArt = trainedUnit.getFieldAsString(ABIL_ART, 0);
                        ...
                        if(key.equals("R00T")) {
                            ...
                            System.err.println("Lookup to data got " + trainedUnit.getFieldAsString(menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.UPGRADES ? War3ID.fromString("gnam") : menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.ABILITIES ? War3ID.fromString("anam") : War3ID.fromString("unam"), 0) + " to (" + xPosition + "," + yPosition + ")");

                        }
                        while (xPosition >= 0 && xPosition <= 3 && yPosition >= 0 && yPosition <= 2 && usedButtonPositions[yPosition][xPosition]) {
                            xPosition++;
                            if (xPosition > 3) {
                                xPosition = 0;
                                yPosition++;
                        ...
                        if (xPosition >= 0 && xPosition <= 3 && yPosition >= 0 && yPosition <= 2) {
                            if (unitId.toString().equals("h013")) {
                                System.err.println("Assigning " + trainedUnit.getFieldAsString(menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.UPGRADES ? War3ID.fromString("gnam") : menuMakingKey.getDataType() == MutableObjectData.WorldEditorDataType.ABILITIES ? War3ID.fromString("anam") : War3ID.fromString("unam"), 0) + " to (" + xPosition + "," + yPosition + ")");
                            }
        ...
        for (final War3ID unitId : objectData.getUnits().keySet()) {
            final MutableGameObject unitType = objectData.getUnits().get(unitId);
            Integer xPositionCached = objectToXCoord.get(unitType);
            Integer yPositionCached = objectToYCoord.get(unitType);
            int xPosition = xPositionCached != null ? xPositionCached : unitType.getFieldAsInteger(UNIT_BUTTON_X, 0);
            int yPosition = yPositionCached != null ? yPositionCached : unitType.getFieldAsInteger(UNIT_BUTTON_Y, 0);
            String tooltip = unitType.getFieldAsString(UNIT_TOOLTIP, 0).replace("|r", "").replace("|R", "");
            String gridHotkey = "_";
            if (xPosition >= 0 && yPosition >= 0 && yPosition < grid.length && xPosition < grid[yPosition].length) {
        ...
        for (final War3ID unitId : objectData.getItems().keySet()) {
            ...
            Integer xPositionCached = objectToXCoord.get(unitType);
            Integer yPositionCached = objectToYCoord.get(unitType);
            int xPosition = xPositionCached != null ? xPositionCached : unitType.getFieldAsInteger(ITEM_BUTTON_X, 0);
            int yPosition = yPositionCached != null ? yPositionCached : unitType.getFieldAsInteger(ITEM_BUTTON_Y, 0);
            String tooltip = unitType.getFieldAsString(UNIT_TOOLTIP, 0).replace("|r", "").replace("|R", "");
            String gridHotkey = "_";
            if (xPosition >= 0 && yPosition >= 0 && yPosition < grid.length && xPosition < grid[yPosition].length) {
                gridHotkey = grid[yPosition][xPosition];
        ...
        for (final War3ID unitId : objectData.getAbilities().keySet()) {
            final MutableGameObject unitType = objectData.getAbilities().get(unitId);
            {
                Integer xPositionCached = objectToXCoord.get(unitType);
                Integer yPositionCached = objectToYCoord.get(unitType);
                int xPosition = xPositionCached != null ? xPositionCached : unitType.getFieldAsInteger(ABIL_BUTTON_X, 0);
                int yPosition = yPositionCached != null ? yPositionCached : unitType.getFieldAsInteger(ABIL_BUTTON_Y, 0);
                String gridHotkey = "_";
                if (xPosition >= 0 && yPosition >= 0 && yPosition < grid.length && xPosition < grid[yPosition].length) {
                    gridHotkey = grid[yPosition][xPosition];
                }
            ...
            {
                Integer xPositionCached = objectToLearnXCoord.get(unitType);
                Integer yPositionCached = objectToLearnYCoord.get(unitType);
                int xPosition = xPositionCached != null ? xPositionCached : unitType.getFieldAsInteger(ABIL_LEARN_BUTTON_X, 0);
                int yPosition = yPositionCached != null ? yPositionCached : unitType.getFieldAsInteger(ABIL_LEARN_BUTTON_Y, 0);
                String gridHotkey = "_";
                if (xPosition >= 0 && yPosition >= 0 && yPosition < grid.length && xPosition < grid[yPosition].length) {
                    gridHotkey = grid[yPosition][xPosition];
                }
        ...
        for (final War3ID unitId : objectData.getUpgrades().keySet()) {
            ...
            Integer xPositionCached = objectToXCoord.get(unitType);
            Integer yPositionCached = objectToYCoord.get(unitType);
            int levels = unitType.getFieldAsInteger(UPGR_LEVELS, 0);
            for (int i = 1; i <= levels; i++) {
                int xPosition = xPositionCached != null ? xPositionCached : unitType.getFieldAsInteger(UPGR_BUTTON_X, i);
                int yPosition = yPositionCached != null ? yPositionCached : unitType.getFieldAsInteger(UPGR_BUTTON_Y, i);
                String gridHotkey = "_";
                if (xPosition >= 0 && yPosition >= 0 && yPosition < grid.length && xPosition < grid[yPosition].length) {
                    gridHotkey = grid[yPosition][xPosition];
                }
        ...
        try {
            try (BlizzardDataOutputStream outputStream = new BlizzardDataOutputStream(new File("C:/Users/micro/OneDrive/Documents/Warcraft III/Maps/Altered Melee/HFGen/out.w3u"), false)) {
                objectData.getUnits().getEditorData().save(outputStream, false);
            }
            try (BlizzardDataOutputStream outputStream = new BlizzardDataOutputStream(new File("C:/Users/micro/OneDrive/Documents/Warcraft III/Maps/Altered Melee/HFGen/out.w3t"), false)) {
                objectData.getItems().getEditorData().save(outputStream, false);
            }
            try (BlizzardDataOutputStream outputStream = new BlizzardDataOutputStream(new File("C:/Users/micro/OneDrive/Documents/Warcraft III/Maps/Altered Melee/HFGen/out.w3a"), false)) {
                objectData.getAbilities().getEditorData().save(outputStream, false);
            }
            try (BlizzardDataOutputStream outputStream = new BlizzardDataOutputStream(new File("C:/Users/micro/OneDrive/Documents/Warcraft III/Maps/Altered Melee/HFGen/out.w3q"), false)) {
    ...
    private static String processTooltip(String gridHotkey, String tooltip) {
        while (hasColorCodes(tooltip)) {
            int colorIndex = tooltip.toLowerCase().indexOf("|c");
            tooltip = tooltip.substring(0, colorIndex) + tooltip.substring(colorIndex + 10);
        }
        if (tooltip.contains(" (") && tooltip.contains(")")) {
            int openPar = tooltip.lastIndexOf("(");
            int closePar = tooltip.lastIndexOf(")");
            if (closePar - openPar == 2) {
                // Strip off the " (Q)" suffix from the Tinker
                tooltip = tooltip.substring(0, openPar - 1);
            }
        }
        int indexOfLevelInfo = tooltip.indexOf(" - [");
        if(indexOfLevelInfo != -1) {
            tooltip = tooltip.substring(0, indexOfLevelInfo) + " (|cffffcc00" + gridHotkey + "|r)" + tooltip.substring(indexOfLevelInfo);
        } else {
    ...
    private static final class UILookupField {
        ...
        private boolean sharedCommandCard;

        public UILookupField(War3ID fieldMetaKey, MutableObjectData.WorldEditorDataType dataType, boolean sharedCommandCard) {
