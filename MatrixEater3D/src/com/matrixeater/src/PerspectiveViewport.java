...
//					textureMapCID.put(tex, geo.material.layers.get(i).getCoordId());
	...
	public void initGL() {
    	try {
    	if( MainFrame.panel == null || MainFrame.panel.textureModels() )
    	{
    		texLoaded = true;
    		for (Geoset geo : dispMDL.getMDL().m_geosets) {//.getMDL().m_geosets
    			for( int i = 0; i < geo.material.layers.size(); i++ )
...
//        GL11.glLoadIdentity();
    ...
    {
    	return texLoaded && (MainFrame.panel == null || MainFrame.panel.textureModels());
    }
...
//    	setSize(getParent().getSize());
    	...
    	else if( !texLoaded && (MainFrame.panel == null || MainFrame.panel.textureModels()) )
    	{
			forceReloadTextures();
			texLoaded = true;
    	}
        try { 
            ...
            if(  MainFrame.panel != null && MainFrame.panel.viewMode() == 0 )//wireframe.isSelected() )
            	glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
            else if(  MainFrame.panel == null || MainFrame.panel.viewMode() == 1 )
...
//				}
            ...
            GL11.glDisable(GL11.GL_TEXTURE_2D);
			if( MainFrame.panel != null && MainFrame.panel.showNormals() )
			{
