...
import com.requestin8r.src.units.ModelOptionPane;
import com.requestin8r.src.units.Unit;
import com.requestin8r.src.units.UnitOptionPane;
/**
...
public class MainPanel extends JPanel implements ActionListener, MouseListener, ChangeListener
{
    ...
    JMenuBar menuBar;
    JMenu fileMenu, recentMenu, editMenu, modelMenu, mirrorSubmenu, viewMenu, addMenu, aboutMenu, fetch;
    JCheckBoxMenuItem mirrorFlip, showNormals, textureModels, showVertexModifyControls;
    ArrayList geoItems = new ArrayList();
    JMenuItem open, fetchUnit, fetchModel, save, showController, mergeGeoset, saveAs, importButton, importButtonS, newDirectory, creditsButton, clearRecent, nullmodelButton, selectAll, invertSelect, expandSelection, mirrorX, mirrorY, mirrorZ, insideOut, showMatrices, editUVs, scaleAnimations, riseFallBirth, teamColor, teamGlow, addParticle;
...
//         JPanel root = this;
    ...
    public JMenuBar createMenuBar()
    {
        ...
        riseFallBirth = new JMenuItem("Add Rising/Falling Birth/Death");
        riseFallBirth.setMnemonic(KeyEvent.VK_R);
        riseFallBirth.addActionListener(this);
        modelMenu.add(riseFallBirth);
...
//             }
    			...
    			Unit choice = UnitOptionPane.show(this);
    			if( choice != null ) {
    			...
    			try {
    				...
    				exc.printStackTrace();
    				//bad model!
    				JOptionPane.showMessageDialog(MainFrame.frame,"The chosen model could not be used.","Program Error",JOptionPane.ERROR_MESSAGE);
    			}
    		}
        ...
        else if( e.getSource() == saveAs )
        {
            ...
            if( returnValue == JFileChooser.APPROVE_OPTION )
            {
                if( temp != null )
                {
                	...
                	String ext = ff.accept(new File("junk.mdl")) ? ".mdl" : ".mdx";
...
//        		disp.uvpanel.showFrame();
//        	}
        	AnimationFrame aFrame = new AnimationFrame(disp);
        	aFrame.setVisible(true);
        }
        else if( e.getSource() == riseFallBirth )
        {
        	MDLDisplay disp = currentMDLDisp();
        	MDL model = disp.getMDL();
        	Animation lastAnim = model.getAnim(model.getAnimsSize()-1);
        	
        	Animation oldBirth = model.findAnimByName("birth");
        	Animation oldDeath = model.findAnimByName("death");
        	...
        	Animation stand = model.findAnimByName("stand");
        	
        	if( oldBirth != null ) {
        		String[] choices = {"Ignore", "Delete", "Overwrite"};
        		Object x = JOptionPane.showInputDialog(this, "Existing birth detected. What should be done with it?", "Question", JOptionPane.PLAIN_MESSAGE, null, choices, choices[0]);
        		if( x == choices[1] ) {
        	...
        	if( oldDeath != null ) {
        		String[] choices = {"Ignore", "Delete", "Overwrite"};
        		Object x = JOptionPane.showInputDialog(this, "Existing death detected. What should be done with it?", "Question", JOptionPane.PLAIN_MESSAGE, null, choices, choices[0]);
        		if( x == choices[1] ) {
        	...
        	List<IdObject> roots = new ArrayList<IdObject>();
        	...
        	// visibility
        	for( VisibilitySource source: model.getAllVisibilitySources() ) {
	        	AnimFlag dummy = new AnimFlag("dummy");
        		AnimFlag af = source.getVisibilityFlag();
        		dummy.copyFrom(af);
        		af.deleteAnim(birth);
        		af.deleteAnim(death);
...
//    	                 botArea.clearGeosets();
    ...
    public void loadFile(File f, boolean temporary) {
