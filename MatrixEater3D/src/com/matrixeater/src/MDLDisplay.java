    ...
    public void drawGeosets(Graphics g, UVViewport vp, int vertexSize)
    {
        ...
        for( Geoset geo: model.m_geosets )
        {
            if( geo == highlight )
            {
                ...
                geo.drawTriangles(g,vp,uvpanel.currentLayer());
                g.setColor( new Color( 0, 255, 0 ) );
                geo.drawTVerteces(g,vp,vertexSize,uvpanel.currentLayer());
            }
        }
    }
...
//             }
    ...
    public void manualMove()
    {
        if( !lockdown )
        {
            ...
            int x = JOptionPane.showConfirmDialog(this.mpanel, inputPanel, "Manual Translation", JOptionPane.OK_CANCEL_OPTION);
            if (x != JOptionPane.OK_OPTION) {
            	return;
            }
            double deltaX = ((Number)spinners[0].getValue()).doubleValue();
            double deltaY = ((Number)spinners[1].getValue()).doubleValue();
            double deltaZ = ((Number)spinners[2].getValue()).doubleValue();
            for( Vertex ver: selection )
            {
            ...
            //updateAction(new Vertex(0,0,0),((MoveAction)currentAction).moveVector,0,1);
            redoStack.clear();
    ...
    public void manualSet()
    {
        if( !lockdown && selection.size() > 0 )
        {
            ...
            double deltaY = ((Number)spinners[1].getValue()).doubleValue();
            double deltaZ = ((Number)spinners[2].getValue()).doubleValue();
            Vertex centerOfGravity = Vertex.centerOfGroup(selection);
            Vertex changeVector = new Vertex(deltaX-centerOfGravity.x, deltaY-centerOfGravity.y, deltaZ-centerOfGravity.z);
            for( Vertex ver: selection )
            {
...
//                	}
    ...
    public void setBeenSaved(boolean flag) {
    	beenSaved = flag;
    }
