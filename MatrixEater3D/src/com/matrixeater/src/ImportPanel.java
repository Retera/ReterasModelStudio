                ...
                if( geosetAnimTabs.isEnabledAt(i) )
                {
                    ...
                    for( int l = 0; l < bap.oldBoneRefs.size(); l++ )
                    {
                        ...
                        if( ms.matrix.bones.size() < 1 ) {
                        	...
                        	if( !shownEmpty ) {
                        		JOptionPane.showMessageDialog(null, "Warning: You left some matrices empty. This was detected, and a dummy bone at { 0, 0, 0 } has been generated for them named " + dummyBone.getName() +"\nMultiple geosets may be attached to this bone, and the error will only be reported once for your convenience.");
                        		shownEmpty = true;
                        	}
                        	if( !ms.matrix.bones.contains(dummyBone) ) {
                        		ms.matrix.bones.add(dummyBone);
                        	}
                        }
...
//                     if( test )
    	...
    	for( int i = 0; i < visSourcesNew.size() && corpseShell == null; i++ )
    	{
        	if( visSourcesNew.get(i) instanceof VisibilityShell )
        	{
        		...
        		if( vs.source instanceof Geoset )
        		{
        			...
        			if( g.geosetAnim != null && g.material.firstLayer().firstTexture().getPath().toLowerCase().equals("textures\\gutz.blp"))
        			{
        				corpseShell = vs;
        				//JOptionPane.showMessageDialog(null,"Anim file corpse found at index "+i);
        			}
        		}
        	}
    	}
    	//JOptionPane.showMessageDialog(null,"AutoTrans corpse visibility:");
    	//JOptionPane.showMessageDialog(null,(corpseShell != null) + ", "+visComponents.size());
    	if( corpseShell != null )
    	{
    		for(int i = 0; i < visComponents.getSize(); i++ )
    		{
    			VisibilityPane vp = visComponents.get(i);
    			//JOptionPane.showMessageDialog(null,"Visibility component: "+i +", "+vp.sourceShell.source.getName());
    			if( vp.sourceShell.source instanceof Geoset)
    			{
        	    	//System.err.println("Geoset at index: "+i);
            		Geoset g = (Geoset)vp.sourceShell.source;
    				//JOptionPane.showMessageDialog(null,"Source file geo tex: "+g.material.firstLayer().firstTexture().getPath());
            		if( g.geosetAnim != null && g.material.firstLayer().firstTexture().getPath().toLowerCase().equals("textures\\gutz.blp"))
            		{
            			vp.newSourcesBox.setSelectedItem(corpseShell);
        				//JOptionPane.showMessageDialog(null,"Source file corpse set at index "+i);
            		}
    			}
    		}
    	}
    ...
    //*********************Simple Import Functions****************
    public void animTransferPartTwo(boolean singleAnimation, Animation pickedAnim, Animation visFromAnim, boolean show)
    {
    		...
    		for( int i = 0; i < animTabs.getTabCount(); i++ ) {
                ...
                if( aniPanel.anim.getName().equals(visFromAnim.getName()) ) {
                	...
                	for( int d = 0; d < existingAnims.getSize(); d++ ) {
                		AnimShell shell = (AnimShell)existingAnims.get(d);
        ...
        for( int i = 0; i < listModel.size(); i++ )
        {
            ...
            if( bs.bone.getName().equals(bone.getName()) && bs.importBone == null
            		&& (!(bs.bone.getName().contains("Mesh")
            			...
            			|| bs.bone.pivotPoint.equalLocs(bone.pivotPoint) )
            		)
            {
