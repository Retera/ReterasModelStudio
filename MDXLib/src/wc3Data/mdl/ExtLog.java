...
import wc3Data.mdx.GeosetChunk;
...
public class ExtLog
{
    ...
    public ExtLog(GeosetChunk.Geoset.Extent source) {
    ...
    public static ExtLog parseText(String [] line)
    {
        if( line[0].contains("Extent") || line[0].contains("BoundsRadius")   )
        {
            ...
            for( int i = 0; i < line.length; i++ )
            {
                ...
                else if( line[i].contains("BoundsRadius"))
                {
                    ...
                    {
                        JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error {"+s+"}: BoundsRadius could not be interpreted.");
                    }
                }
                else
                {
                    JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse ExtLog; unrecognized input: "+line[i]);
                }
            }
            return extLog;
        }
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse ExtLog: Missing or unrecognized open statement.");
        }
        return null;
    }
    public static ExtLog read(BufferedReader mdl)
    {
        ...
        if( line.contains("Extent") || line.contains("BoundsRadius")   )
        {
            ...
            while( !((line).contains("\t}")) && (line.contains("Extent") || line.contains("BoundsRadius"))  )
            {
                ...
                {
                    JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse ExtLog; unrecognized input: "+line);
                }
                MDLReader.mark(mdl);
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse ExtLog: Missing or unrecognized open statement: "+line);
        }
        return null;
    }
