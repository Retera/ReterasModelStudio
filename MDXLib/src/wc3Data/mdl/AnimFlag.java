...
import wc3Data.mdx.MaterialAlpha;
import wc3Data.mdx.MaterialTextureId;
import wc3Data.mdx.TextureRotation;
import wc3Data.mdx.TextureScaling;
import wc3Data.mdx.TextureTranslation;
/**
...
{
	public static String getInterpType(int id) {
		switch (id) {
		case 0:
			return "DontInterp";
		case 1:
    ...
    private static Double box(float f) {
    	return new Double(f);
    }
    public AnimFlag(MaterialTextureId source) {
    	title = "TextureID";
		generateTypeId();
		...
		boolean tans = source.interpolationType > 1;
		for( MaterialTextureId.ScalingTrack track: source.scalingTrack ) {
			if( tans ) {
    ...
    public AnimFlag(MaterialAlpha source) {
    	title = "Alpha";
		generateTypeId();
		addTag(AnimFlag.getInterpType(source.interpolationType));
		if(source.globalSequenceId >= 0 ) {
		...
		boolean tans = source.interpolationType > 1;
		for( MaterialAlpha.ScalingTrack track: source.scalingTrack ) {
			if( tans ) {
    ...
    public AnimFlag(TextureRotation textureData) {
    	title = "Rotation";
		generateTypeId();
		...
		boolean tans = textureData.interpolationType > 0;
		for( TextureRotation.TranslationTrack track: textureData.translationTrack ) {
			if( tans ) {
    ...
    public AnimFlag(TextureScaling textureData) {
    	title = "Scaling";
		generateTypeId();
		addTag(AnimFlag.getInterpType(textureData.interpolationType));
		if(textureData.globalSequenceId >= 0 ) {
		...
		boolean tans = textureData.interpolationType > 0;
		for( TextureScaling.TranslationTrack track: textureData.translationTrack ) {
			if( tans ) {
    ...
    public AnimFlag(TextureTranslation textureData) {
    	title = "Translation";
		generateTypeId();
		addTag(AnimFlag.getInterpType(textureData.interpolationType));
		if(textureData.globalSequenceId >= 0 ) {
		...
		boolean tans = textureData.interpolationType > 0;
		for( TextureTranslation.TranslationTrack track: textureData.translationTrack ) {
			if( tans ) {
	...
	public void addEntry(Integer time, Object value) {
		times.add(time);
		values.add(value);
	}
	public void addEntry(Integer time, Object value, Object inTan, Object outTan) {
		times.add(time);
		values.add(value);
		inTans.add(inTan);
		outTans.add(outTan);
