...
//    	m_cameras = new ArrayList(other.m_cameras);
    ...
    public void loadFile(File f)
    {
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Attempted to read file, but file was not found.");
            return;
        }
        String line = "opening";
        loading = true;
        ...
        while( loading )
        {
            ...
            {
                JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error: Vertices not found at beginning of Geoset "+geoId+"!");
            }
            ...
            line = nextLine(mdl);
            //JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Triangles found on line"+c);
            if( loading )
            {
    ...
    public void parseVertex(String input, Geoset geoset)
    {
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error (on line "+c+"): Vertex coordinates could not be interpreted.");
        }
    }
    ...
    public static MDL read(File f)
    {
    	...
    	try {
            ...
            {                              // gotta have that sense of humor, right?
                JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Model could not be understood. Program does not understand this type of file.");
            }
            line = MDLReader.nextLine(mdl);
...
//    		JOptionPane.showMessageDialog(null,newJTextPane(e));
    ...
    public void doPostRead() {
        ...
        for( Geoset geo: m_geosets )
        {
            geo.updateToObjects(this);
        }
        for( GeosetAnim geoAnim: this.m_geosetanims )
        {
            if( geoAnim.geosetId != -1 )
            {
        ...
        List<AnimFlag> animFlags = this.getAllAnimFlags();//laggggg!
        System.out.println("AnimFlags:");
        for( AnimFlag af: animFlags )
        {
    ...
    public void printTo(File baseFile)
    {
    	File f = baseFile;
    	boolean mdx = false;
    	if( f.getPath().endsWith(".mdx") ) {
