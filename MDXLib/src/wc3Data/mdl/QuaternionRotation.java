...
//          d = c1 * s2 * c3 - s1 * c2 * s3;
    ...
    public QuaternionRotation( float[] data ) {
    	a = data[0];
    	b = data[1];
    	c = data[2];
    	d = data[3];
...
//         Now Quaternions can go burn and die.
    ...
    public static QuaternionRotation parseText(String input)
    {
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error {"+input+"}: QuaternionRotation coordinates could not be interpreted.");
        }
        try
        {
