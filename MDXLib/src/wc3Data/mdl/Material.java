...
public class Material
{
    ...
    private ArrayList<String> flags = new ArrayList<String>();//My way of dealing with all the stuff that I forget/don't bother with: "Unshaded," "Unfogged," "TwoSided," "CoordId X," actually CoordId was moved into its own field 
    ...
    public Material(List<Layer> layers) {
...
//                     JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error parsing Material: Unrecognized statement '"+line[i]+"'.");
                }
            }
            return mat;
        }
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse Material: Missing or unrecognized open statement.");
        }
        return null;
    }
...
//                     JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error parsing Material: Unrecognized statement '"+line[i]+"'.");
    ...
    public static ArrayList<Material> readAll(BufferedReader mdl, MDL mdlr)
    {
        ...
        {
            MDLReader.reset(mdl);
//             JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse Materials: Missing or unrecognized open statement.");
        }
        return outputs;
    }
...
//				exc.printStackTrace();
//				try {
//					BufferedImage newImage = BLPHandler.get().getCustomTex(MDLReader.getDefaultContainer().currentMDL().getFile().getParent()+"\\"+path);
//					if( theImage == null )
//						theImage = newImage;
//					else
//						theImage = mergeImage(theImage, newImage);
