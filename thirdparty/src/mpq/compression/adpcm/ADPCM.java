...
public class ADPCM {
	...
	public ADPCM(final int channelmax){
	...
	public void decompress(final ByteBuffer in, final ByteBuffer out, final int channeln){
		// prepare buffers
		in.order(ByteOrder.LITTLE_ENDIAN);
		out.order(ByteOrder.LITTLE_ENDIAN);
		
		final byte stepshift = (byte) (in.getShort() >>> 8);
		
		// initialize channels
		for( int i = 0 ; i < channeln ; i+= 1 ){
			final Channel chan = state[i];
			chan.rate = 0x2C;
		...
		final boolean multichannel = channeln > 1;
		...
		while( in.hasRemaining() ){
			...
			if( (op & 0x80) > 0 ){
				...
				// adjust value
				final short stepunit = STEP_TABLE[chan.rate];
