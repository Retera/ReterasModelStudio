...
public class Compression {	
	...
	private ByteBuffer fetchBuffer(final int size){
		ByteBuffer out;
		if( size >= 0 ){
	 ...
	 */
	public ByteBuffer blockDecompress3(final ByteBuffer in, final int size) throws DecompressionException{
		final byte mask = in.get();
	 ...
	 * @throws DecompressionException when decompression fails
	 */
	public ByteBuffer blockDecompress2(final ByteBuffer in, final int size) throws DecompressionException{
		final byte mask = in.get();
		
		final ByteBuffer out = fetchBuffer(size);
		boolean flip = false;
	 ...
	 * @throws DecompressionException when decompression fails
	 */
	public ByteBuffer blockDecompress1(final ByteBuffer in, final int size) throws DecompressionException{
		final ByteBuffer out = fetchBuffer(size);
		sectorExplode(in, out);
		in.clear();
		bufferCache[size] = in;
	...
	private void sectorExplode(final ByteBuffer in, final ByteBuffer out) throws DecompressionException{
		try {
	...
	private void sectorInflate(final ByteBuffer in, final ByteBuffer out) throws DecompressionException{
		try {
			// a new inflater is needed for each sector as they cannot be recycled
			final Inflater zlibInflater = new Inflater();
	...
	private void sectorHuffmanExpand(final ByteBuffer in, final ByteBuffer out) throws DecompressionException{
		try {
	...
	private void sectorADPCMReconstruct(final ByteBuffer in, final ByteBuffer out, final int channeln) throws DecompressionException{
		try {
	...
	public ByteBuffer blockDecompressAny(final ByteBuffer block, final ByteBuffer extra) throws DecompressionException{
		if( blockDecompress(block, extra) ) {
			return extra;
		} else {
			return block;
		}
	...
	public void blockExplode(final ByteBuffer block, final ByteBuffer extra){
		try {
	...
	public boolean blockDecompress(final ByteBuffer block, final ByteBuffer extra) throws DecompressionException{
		return blockDecompress(block, extra, false);
	}
	
	public boolean blockDecompress(ByteBuffer block, ByteBuffer extra, final boolean strict) throws DecompressionException{
		final byte mask = block.get();
