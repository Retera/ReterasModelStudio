...
public class PKExploder {
	...
	private static void denDecodeTabs(final byte[] pos, final short[] sindex, final byte[] len, final int size){
		for( byte i = 0 ; i < size ; i++ ){
	        final short length = (short) (1 << len[i]);
	...
	private void WasteBits(final byte nbits){
	...
	private void expand() throws PKException{
		for( ; ; ){
			if( (bit_buff & 0x01) != 0 ){
				...
				final byte extra_length_bits;
				if ((extra_length_bits = EX_LEN_BITS[length_code]) != 0) {
					final byte extra_length = (byte) (bit_buff & ((1 << extra_length_bits) - 1));

					try {
						WasteBits(extra_length_bits);
					} catch (final BufferUnderflowException e) {
						if ((length_code + (extra_length & 0xFF)) == 0x10E) {
							return;
						} else {
							throw e;
						}
		        ...
		        // get distance
		        final byte dist_pos_code = DIST_POS_CODES[bit_buff & 0xFF];
		        final byte dist_pos_bits = DIST_BITS[dist_pos_code];
				...
				case CMP_BINARY:
					// read raw byte
					final byte uncompressed_byte = (byte) (bit_buff & 0xFF);
					WasteBits((byte) 8);
	...
	public void explode(final ByteBuffer in, final ByteBuffer out) throws PKException{
