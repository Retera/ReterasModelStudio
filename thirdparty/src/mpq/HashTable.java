...
public class HashTable {
	...
	private final Entry[] bucketArray;

	// raw constructor, assumes every entry is not null and the array is a power of 2
	public HashTable(final Entry[] entries) {
		bucketArray = entries;
	}

	public int lookupBlock(final HashLookup what) throws MPQException {
		final int mask = bucketArray.length - 1;
		final int index = what.index & mask;
		for (int pos = index; ; ) {
			final Entry temp = bucketArray[pos];
			if(temp.blockIndex == BLOCK_EMPTY_ALWAYS) {
				break;
			}
			if(temp.getHash() == what.hash) {
				return temp.blockIndex;
			}
			pos = ( pos + 1 ) & mask;
			if(pos == index) {
				break;
	...
	public static class Entry{
		...
		public Entry(final HashTableEntry source){
			hash = source.getHash();
