...
public class UnitEditorModelSelector extends JSplitPane implements TreeSelectionListener {
	...
	MDL mdl = new MDL();
	//MDL mdl;
	MDLDisplay modelDisp = new MDLDisplay(mdl,null);
...
//		temp.setBackground(Color.blue);
		...
		modelPanel = new PerspDisplayPanel("blank",modelDisp);
...
//		KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(myKeyEventDispatcher);
	...
	public void fillTable() {
		...
		try {
        	...
        	try (BlizzardDataInputStream reader = new BlizzardDataInputStream(MpqCodebase.get().getResourceAsStream(filepath))) {
            	mdl = MdxUtils.loadModel(reader).toMDL();
            	modelDisp = new MDLDisplay(mdl,null);
...
//            modelMenu.setEnabled(true);
			...
			exc.printStackTrace();
			//bad model!
			JOptionPane.showMessageDialog(getParent(),"The chosen model could not be used.","Program Error",JOptionPane.ERROR_MESSAGE);
		}
	...
	public void loadHotkeys() {
		...
		this.getRootPane().getActionMap().put("displayAsRawData", new AbstractAction() {
			...
			public void actionPerformed(final ActionEvent e) {
		        ...
		        final Enumeration<DefaultMutableTreeNode> enumer = UnitEditorModelSelector.this.root.breadthFirstEnumeration();
