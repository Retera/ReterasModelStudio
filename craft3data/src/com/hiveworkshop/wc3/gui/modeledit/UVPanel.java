...
//				}
    ...
    public JMenuBar createMenuBar() {
        ...
        JMenuItem splitVertex = new JMenuItem("Split Vertex");
        ...
        splitVertex.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (mpanel != null) {
                    ...
                    Collection<? extends TVertex> selectedTVertices = modelEditorManager.getSelectionView().getSelectedTVertices(currentLayer());
                    for(Geoset g: mpanel.getModel().getGeosets()) {
                        for(GeosetVertex gv: new ArrayList<>(g.getVertices())) {
                            TVertex tVertex = gv.getTVertex(currentLayer());
                    ...
                    /*Collection<Triangle> selectedFaces = modelEditorManager.getSelectionView().getSelectedFaces();
                    for(Triangle t: selectedFaces) {
                        for(int i = 0; i < 3; i++) {
                            GeosetVertex gv = t.get(i);
                            if(gv.getTriangles().size()>1) {
                                GeosetVertex copy = new GeosetVertex(gv);
                                t.set(i, copy);
                                copy.getTriangles().add(t);
                            }
                        }
                    }*/
            ...
            if (x == JOptionPane.YES_OPTION) {
                ...
                if (materialsList.getSelectedValue() != null) {
                    vp.addBackgroundImage(materialsList.getSelectedValue()
                            .getBufferedImage(dispMDL.getModel().getWorkingDirectory()));
                }
