		...
		public void save(BlizzardDataOutputStream out) throws IOException {
			...
			if (rotation.length % 4 != 0) {
				throw new IllegalArgumentException(
						LocalizationManager.getInstance().get("exception.geosetrotation_save_array_rotation")
								+ rotation.length + ")");
			}
			...
			if (interpolationType > 1) {
				if (inTan.length % 4 != 0) {
					throw new IllegalArgumentException(
							LocalizationManager.getInstance().get("exception.geosetrotation_save_array_intan")
									+ inTan.length + ")");
				}
				MdxUtils.saveFloatArray(out, inTan);
				if (outTan.length % 4 != 0) {
					throw new IllegalArgumentException(
							LocalizationManager.getInstance().get("exception.geosetrotation_save_array_outtan")
									+ outTan.length + ")");
				}
