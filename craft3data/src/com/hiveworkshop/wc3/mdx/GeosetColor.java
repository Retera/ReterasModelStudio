		...
		public void save(BlizzardDataOutputStream out) throws IOException {
			...
			if (color.length % 3 != 0) {
				throw new IllegalArgumentException(
						LocalizationManager.getInstance().get("exception.geosetcolor_save_array_color")
								+ color.length + ")");
			}
			...
			if (interpolationType > 1) {
				if (inTan.length % 3 != 0) {
					throw new IllegalArgumentException(
							LocalizationManager.getInstance().get("exception.geosetcolor_save_array_intan")
									+ inTan.length + ")");
				}
				MdxUtils.saveFloatArray(out, inTan);
				if (outTan.length % 3 != 0) {
					throw new IllegalArgumentException(
							LocalizationManager.getInstance().get("exception.geosetcolor_save_array_outtan")
									+ outTan.length + ")");
				}
