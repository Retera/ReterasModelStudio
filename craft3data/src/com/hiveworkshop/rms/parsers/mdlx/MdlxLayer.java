...
import com.hiveworkshop.rms.parsers.mdlx.FilterMode;
	...
	public void readMdx(final BinaryReader reader, final int version) {
		...
		while (0 < tempSize) {
			...
			if (timeline != null) {
				...
				MdlxParseLog.debug("couldn't find tag for: " + name);
				break;
			}
		}
		...
		for (; sizeTracker< size; sizeTracker+=4) {
				MdlxParseLog.debug(sizeTracker/4 + " (" + sizeTracker + "): " + reader.readInt32());
		}
	}
	...
	public void readMdl(final MdlTokenInputStream stream, final int version) {
		...
		while (iterator.hasNext()) {
			...
			switch (token) {
				...
				case MdlUtils.TOKEN_BACK_FACES_FOR_SHADOWS -> flags |= 0x200;
				case MdlUtils.TOKEN_AMBIENT_OCCLUSION -> flags |= 0x400;
				case MdlUtils.TOKEN_SHADER -> hdFlag = shaderNameToHdFlag(stream.read());
				case MdlUtils.TOKEN_STATIC_TEXTURE_ID -> {
					// `static TextureID id,` or the game's `static TextureID id <= slot,`
					textureId = stream.readInt();
					int slot = hdTextureSlots.size();
					if (MdlUtils.TOKEN_TEXTURE_SLOT_DESIGNATOR.equals(stream.peek())) {
						stream.read();
						slot = stream.readInt();
					}
					hdTextureIds.add(textureId);
					hdTextureSlots.add(slot);
				}
				case MdlUtils.TOKEN_TEXTURE_ID -> {
					// `TextureID n {` or the game's `TextureID n <= slot {`
					...
					timeline.readMdl(stream, keyFrameCount);
...
//					}
				...
				default ->
						MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Layer: " + token);
			}
		}
	}
	...
	public void writeMdl(final MdlTokenOutputStream stream, final int version) {
		...
		if ((flags & 0x200) != 0 && MdlxVersion.isForsakenKingdom(version)) stream.writeFlag(MdlUtils.TOKEN_BACK_FACES_FOR_SHADOWS);
		if ((flags & 0x400) != 0 && MdlxVersion.isForsakenKingdom(version)) stream.writeFlag(MdlUtils.TOKEN_AMBIENT_OCCLUSION);

		if (MdlxVersion.writesGameMdlDialect(version)) {
			// the game's dialect: per-layer shader name, every slot bound with its designator
			final String shaderName = hdFlagToShaderName(hdFlag);
			if (shaderName != null) {
				stream.writeStringAttrib(MdlUtils.TOKEN_SHADER, shaderName);
			}
			for (int i = 0; i < hdTextureIds.size(); i++) {
				final int slot = i < hdTextureSlots.size() ? hdTextureSlots.get(i) : i;
				final String designator = " " + MdlUtils.TOKEN_TEXTURE_SLOT_DESIGNATOR + " " + slot;
				if (textureIdTimelineMap.get(i) != null) {
					textureIdTimelineMap.get(i).writeMdl(stream, slot == 0 ? "" : designator);
				} else {
					stream.writeAttrib(MdlUtils.TOKEN_STATIC_TEXTURE_ID, hdTextureIds.get(i) + designator);
				}
			}
		} else if (1000 < version) {
			for (int i = 0; i < hdTextureIds.size(); i++) {
				if (textureIdTimelineMap.get(i) != null) {
	...
	/** The game matches its registered shader names case-insensitively; unknown names are the SD shader (0). */
	public static int shaderNameToHdFlag(final String shaderName) {
		if (MdlUtils.SHADER_HD_DEFAULT_UNIT.equalsIgnoreCase(shaderName)) {
			return 1;
		}
		if (MdlUtils.SHADER_HD_CRYSTAL.equalsIgnoreCase(shaderName)) {
			return 24;
		}
		if (MdlUtils.SHADER_SD_FIXED_FUNCTION.equalsIgnoreCase(shaderName)) {
			return 2;
		}
		return 0;
	}

	/** @return the shader name for the game's MDL dialect, or null for the plain SD shader (which is omitted) */
	public static String hdFlagToShaderName(final int hdFlag) {
		return switch (hdFlag) {
			case 1 -> MdlUtils.SHADER_HD_DEFAULT_UNIT;
			case 24 -> MdlUtils.SHADER_HD_CRYSTAL;
			case 2 -> MdlUtils.SHADER_SD_FIXED_FUNCTION;
			default -> null;
		};
	}
