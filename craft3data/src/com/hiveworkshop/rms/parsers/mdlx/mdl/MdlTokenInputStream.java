...
//		return read5();
	...
	private void printToke(StringBuilder token, int line) {
		if(759 <= line && line <= 761 || 18870 <= line && line <= 18900  || 251067 <= line && line <= 251070 ){
			MdlxParseLog.debug(line + " current token: " + token);
		}
	}
...
//				MdlxParseLog.debug("read(): " + ugg + " i: " + i);
			}
			skipToken("}");
		}
		skipToken("}");
...
//			MdlxParseLog.debug(line + " did not skip, " + peek + " != " + token);
			this.index = index;
			this.line = line;
		}
//		else {
//			MdlxParseLog.debug(line + " skipped, " + peek + " == " + token);
//		}
	}
