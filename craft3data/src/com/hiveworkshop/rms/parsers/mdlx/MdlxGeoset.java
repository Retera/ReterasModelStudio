...
//		// 9: polygons
		...
		if (800 < version && id != UVAS.getValue()) {
			...
			if (id == SKIN.getValue()) {
				final int skinCount = reader.readInt32();
				if (MdlxVersion.hasSkin16(version)) {
					// 3.0: 4 uint16 bone indices + 4 uint16 weights per vertex; indices above 32767 wrap to
					// negative shorts here and are masked back with & 0xFFFF by every reader
					final int[] wide = reader.readUInt16Array(skinCount);
					skin = new short[skinCount];
					for (int i = 0; i < skinCount; i++) {
						skin[i] = (short) wide[i];
					}
				} else {
					skin = reader.readUInt8Array(skinCount);
				}
...
//		MdlxParseLog.debug("Save: vertexCount: " + vertices.length / 3);
//		MdlxParseLog.debug("Save: normalCount: " + normals.length / 3);
//		MdlxParseLog.debug("Save: PTYP.getValue(): " + PTYP.getValue());
//		MdlxParseLog.debug("Save: faceTypeGroupsCount: " + faceTypeGroups.length);
//		MdlxParseLog.debug("Save: faceTypeGroups: " + Arrays.toString(faceTypeGroups));
//		MdlxParseLog.debug("Save: PCNT.getValue(): " + PCNT.getValue());
//		MdlxParseLog.debug("Save: faceGroupsCount: " + faceGroups.length);
//		MdlxParseLog.debug("Save: faceGroupsCount: " + Arrays.toString(faceGroups));
//		MdlxParseLog.debug("Save: PVTX.getValue(): " + PVTX.getValue());
//		MdlxParseLog.debug("Save: faceCount: " + faces.length);
//		MdlxParseLog.debug("Save: faceCount: " + (int)((long)faces.length));
//		MdlxParseLog.debug("Save: faces: " + Arrays.toString(faces));
//
//		MdlxParseLog.debug("Save: VG len: " + vertexGroups.length);
		...
		if (800 < version) {
			...
			if (skin != null) {
				...
				if (MdlxVersion.hasSkin16(version)) {
					final int[] wide = new int[skin.length];
					for (int i = 0; i < skin.length; i++) {
						wide[i] = skin[i] & 0xFFFF;
					}
					writer.writeUInt16Array(wide);
				} else {
...
//			MdlxParseLog.debug("Token: " + token);
			switch (token) {
				...
				default -> MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Geoset: " + token);
			}
		}
	...
	public long getByteLength(final int version) {
		...
		if (800 < version) {
			...
			if (skin != null) {
				size += 8 + skin.length * (MdlxVersion.hasSkin16(version) ? 2L : 1L);
			}
		}
