...
public class MdxLoadSave {
	...
	private static final int DILG = ('D' << 24) | ('I' << 16) | ('L' << 8) | ('G');// 3.0 "Glider" chunk
...
//		MdlxParseLog.debug("LAYS = " + LAYS);
//		MdlxParseLog.debug("VERS = " + VERS
//				+ "\nMODL = " + MODL
//				+ "\nSEQS = " + SEQS
//				+ "\nGLBS = " + GLBS
//				+ "\nMTLS = " + MTLS
...
//				+ "\nBPOS = " + BPOS);
		...
		try {
			MdlxParseLog.debug("bufferSize: " + reader.remaining());
			while (reader.remaining() > 0) {
				...
				try {
					...
					mode = 2;
					MdlxParseLog.debug("tag: " + new War3ID(tag));
			...
			e.printStackTrace();
			MdlxParseLog.display(e);
//			throw new RuntimeException(e);
		}

//		for (int i = 0; i<mdlxModel.textures.size(); i++) {
//			MdlxParseLog.debug("#" + i + ": " + mdlxModel.textures.get(i).path + ", " + mdlxModel.textures.get(i).replaceableId);
//		}
	}
	...
	private static void tryToRecover(MdlxModel mdlxModel, BinaryReader reader, int tag, int size, int mode) {
		...
		int i = 0;
		int j = 0;
		MdlxParseLog.debug("remaining: " + reader.remaining() + "(tag: " + tag + ", size: " + size + ", mode: " + mode + ")");
...
//						tempTag = (tempTag<<8) + reader.readInt8();
			...
			hasFound = switch (testTag) {
				case VERS, MODL, SEQS, GLBS, MTLS, TEXS, TXAN, GEOS, GEOA, BONE, LITE, HELP, ATCH, PIVT, PREM, PRE2, CORN, RIBB, CAMS, EVTS, CLID, FAFX, BPOS, DILG -> true;
				default -> false;
			};
			if (0 <= j && j < 300 && testTag != 0) {
				j++;
				MdlxParseLog.debug(testTag + " (" + tempTag + ") ");
			}
		}
		MdlxParseLog.debug("Geos: " + GEOS);
		MdlxParseLog.debug("tried " + i + "times! testtag: " + testTag + ", temptag:" + tempTag + ", has found: " + hasFound);
		if (reader.remaining() > 32 && hasFound) {
//						int size = reader.readInt32();
			size = reader.readInt32();
			MdlxParseLog.debug("loading tag: " + testTag + " with size: " + size);
	...
	private static void lodByTag(MdlxModel mdlxModel, BinaryReader reader, int tag, int size) {
		MdlxParseLog.debug("reading chunk: " + tag);
		switch (tag) {
			...
			case DILG -> loadGliderChunk(mdlxModel.gliderGeosetIds, reader, size);
	...
	private static <E extends MdlxBlock & MdlxChunk> void readAndPrintChunk(int version, List<E> out, Supplier<E> constructor, BinaryReader reader, long size) {
		...
		MdlxParseLog.debug("first obj size: " + sizeTracker);
		if (sizeTracker< size) {
			...
			MdlxParseLog.debug("second obj size: " + sizeTracker);
		}
		for (int i = sizeTracker; i < size; i+=4) {
			MdlxParseLog.debug(i/4 + " (" + i + "): " + reader.read(4));
		}

		int matEndPos = reader.position();
		MdlxParseLog.debug("read " + (matEndPos - matStartPos) + " bytes");
	}
	...
	private static void loadGliderChunk(List<Integer> gliderGeosetIds, BinaryReader reader, long size) {
		for (long i = 0, l = size / 4; i < l; i++) {
			gliderGeosetIds.add(reader.readInt32());
		}
		reader.move((int) (size - (size / 4) * 4)); // an odd-sized payload: skip the tail
	}
	...
	public static ByteBuffer saveMdx(MdlxModel mdlxModel) {
		...
		if (MdlxVersion.hasGlider(mdlxModel.version) && !mdlxModel.gliderGeosetIds.isEmpty()) {
			writer.writeTag(DILG);
			writer.writeUInt32(mdlxModel.gliderGeosetIds.size() * 4L);
			for (final Integer geosetId : mdlxModel.gliderGeosetIds) {
	...
	public static int getByteLength(MdlxModel mdlxModel) {
		...
		if (MdlxVersion.hasGlider(mdlxModel.version) && !mdlxModel.gliderGeosetIds.isEmpty()) {
			size += 8 + mdlxModel.gliderGeosetIds.size() * 4;
		}
