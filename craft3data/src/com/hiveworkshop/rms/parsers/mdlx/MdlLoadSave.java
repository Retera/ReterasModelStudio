...
public class MdlLoadSave {
		...
		while ((token = stream.read()) != null) {
			try {
				switch (token) {
					...
					case MdlUtils.TOKEN_GLIDER -> loadGliderBlock(mdlxModel.gliderGeosetIds, stream);
					default -> {
						if (token.startsWith("//")) {
							mdlxModel.comments.add(token.replaceFirst("// ?", ""));
						} else if (!token.matches("[\\d.{}\\-eE+]+")) {
							MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown chunk name: " + token);
						}
					}
				}
			} catch (Exception e) {
				MdlxParseLog.setFirstException(e);
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Exception while parsing chunk: " + token + ": " + e.toString());
				e.printStackTrace();
			}
		}
		MdlxParseLog.displayIfNotEmpty();
	}
	...
	private static void loadVersionBlock(MdlxModel mdlxModel, MdlTokenInputStream stream) {
		for (String token : stream.readBlock()) {
			if (MdlUtils.TOKEN_FORMAT_VERSION.equals(token)) {
				...
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Version: " + token);
			}
		}
	}
		...
		for (final String token : stream.readBlock()) {
			if (token.startsWith("Num")) {
				...
				switch (token) {
					...
					default -> MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Model: " + token);
				}
			}
		}
	}
	...
	private static <E extends MdlxBlock> void loadNumberedObjectBlock(int version,
		...
		for (final String token : stream.readBlock()) {
			if (token.equals(name)) {
				...
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in " + name + ": " + token);
			}
		}
	}
	...
	private static void loadGlobalSequenceBlock(List<Long> globalSequences, final MdlTokenInputStream stream) {
		...
		for (final String token : stream.readBlock()) {
			if (token.equals(MdlUtils.TOKEN_DURATION)) {
				...
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in GlobalSequences: " + token);
			}
		}
	}
	...
	private static void loadBindPoseBlock(List<float[]> bindPose, final MdlTokenInputStream stream) {
		for (final String token : stream.readBlock()) {
			if (token.equals(MdlUtils.TOKEN_MATRICES)) {
				...
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in BindPose: " + token);
			}
		}
	}

	/** One block per whitelisted geoset: {@code Glider { GeosetId N, }} (the 3.0 DILG chunk). */
	private static void loadGliderBlock(List<Integer> gliderGeosetIds, final MdlTokenInputStream stream) {
		for (final String token : stream.readBlock()) {
			if (token.equals(MdlUtils.TOKEN_GEOSETID)) {
				gliderGeosetIds.add(stream.readInt());
			} else {
				MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Glider: " + token);
			}
		}
	}
	...
	public static ByteBuffer saveMdl(MdlxModel mdlxModel) {
		...
		if (MdlxVersion.hasGlider(mdlxModel.version)) {
			for (final Integer geosetId : mdlxModel.gliderGeosetIds) {
				stream.startBlock(MdlUtils.TOKEN_GLIDER);
