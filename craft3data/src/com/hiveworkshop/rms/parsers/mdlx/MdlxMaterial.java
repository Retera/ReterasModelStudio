...
public class MdlxMaterial implements MdlxBlock {
	...
	public void readMdx(final BinaryReader reader, final int version) {
		...
		for (int i = sizeTracker; i < size; i+=4) {
			MdlxParseLog.debug("left over in material: " + i/4 + " (" + i + "): " + reader.readInt32());
		}
	}
	...
	public void readMdl(final MdlTokenInputStream stream, final int version) {
		for (final String token : stream.readBlock()) {
			switch (token) {
				...
				default -> MdlxParseLog.addStringToShow("Line " + stream.getLineNumber() + ": Unknown token in Material: " + token);
			}
		}
	}
	...
	public void writeMdl(final MdlTokenOutputStream stream, final int version) {
		...
		if (800 < version && !MdlxVersion.writesGameMdlDialect(version)) stream.writeStringAttrib(MdlUtils.TOKEN_SHADER, shader);
