...
public abstract class MdlxTimeline<TYPE> {
	...
	public void readMdl(final MdlTokenInputStream stream) {
		readMdl(stream, stream.readInt());
	}

	/** Reads the block after its key frame count was already consumed (used for {@code TextureID n <= slot {}). */
	public void readMdl(final MdlTokenInputStream stream, final int keyFrameCount) {
		stream.read(); // {
	...
	/** @param designator text placed after the count, e.g. {@code " <= 3"} for a 3.0 texture slot */
	public void writeMdl(final MdlTokenOutputStream stream, final String designator) {
		final int tracksCount = frames.length;
		stream.startBlock(AnimationMap.ID_TO_TAG.get(name).getMdlToken() + " " + tracksCount + designator);
