...
public class ScriptedConvertForTerrain {
	...
	static File sourceDir = new File("C:\\Warsmash\\Scratch\\ThirdPerson\\Tileset\\Feralas");
	static File destDir = new File("C:\\Warsmash\\Scratch\\ThirdPerson_Tileset\\Tileset\\Feralas");
						...
						for (int pixelX = 0; pixelX < bufferedImg.getWidth(); pixelX++) {
							for (int pixelY = 0; pixelY < bufferedImg.getHeight(); pixelY++) {
								...
								if (bottomRight && !bottomLeft && !topRight && !topleft) {
									...
									alpha = Math.min(1,
											Math.max(0, 1f - (float) new Point2D.Float(fX, fY).distance(1, 0) + 1f
													- (float) new Point2D.Float(fX, fY).distance(0, 1)));
								} else if (bottomRight && bottomLeft && topRight && !topleft) {
									alpha = Math.min(1,
											Math.max(0,
													1f - (float) new Point2D.Float(fX, fY).distance(1, 0) + 1f
															- (float) new Point2D.Float(fX, fY).distance(0, 1) + 1f
															- (float) new Point2D.Float(fX, fY).distance(1, 1)));
								} else if (!bottomRight && !bottomLeft && !topRight && topleft) {
									alpha = Math.max(0, 1f - (float) new Point2D.Float(fX, fY).distance(0, 0));
								} else if (bottomRight && !bottomLeft && !topRight && topleft) {
									alpha = Math.min(1,
											Math.max(0, 1f - (float) new Point2D.Float(fX, fY).distance(1, 1) + 1f
									...
									alpha = Math.min(1,
											Math.max(0,
													1f - (float) new Point2D.Float(fX, fY).distance(0, 0) + 1f
															- (float) new Point2D.Float(fX, fY).distance(0, 1) + 1f
															- (float) new Point2D.Float(fX, fY).distance(1, 1)));
								} else if (!bottomRight && !bottomLeft && topRight && topleft) {
									alpha = 1 - fY;
									...
									alpha = Math.min(1,
											Math.max(0,
													1f - (float) new Point2D.Float(fX, fY).distance(0, 0) + 1f
															- (float) new Point2D.Float(fX, fY).distance(1, 0) + 1f
															- (float) new Point2D.Float(fX, fY).distance(1, 1)));
								} else if (!bottomRight && bottomLeft && topRight && topleft) {
									alpha = Math.min(1,
											Math.max(0,
													1f - (float) new Point2D.Float(fX, fY).distance(0, 0) + 1f
															- (float) new Point2D.Float(fX, fY).distance(0, 1) + 1f
															- (float) new Point2D.Float(fX, fY).distance(1, 0)));
								}
...
//					ImageIO.write(fixed, "blp", destinationFor);
					...
					System.out.println("E"+tileSTr+",-1,Tileset\\Feralas,"+shorterName+","+shorterName+","+shorterName+",1,0,1,1,0,Fdrt,1,0");
