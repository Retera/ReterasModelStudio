...
public class ScriptedConvertForTerrain3 {
	...
	static File sourceDir = new File("C:\\Warsmash\\Scratch\\Zerosmash\\Work\\TerrainArt\\Lorladir");
	static File destDir = new File("C:\\Warsmash\\Scratch\\Zerosmash\\Assets\\TerrainArt\\Lorladir");
	...
	private static final void processFile(File file) {
		if (file.isDirectory()) {
			...
			if (lowerCaseName.endsWith(".tga") && !lowerCaseName.endsWith("_s.blp")) {
				try {
					BufferedImage bufferedImg = TgaFile.readTGA(file);
					...
					System.out.println("E"+tileSTr+",-1,Tileset\\Lorladir,"+shorterName+","+shorterName+","+shorterName+",1,0,1,1,0,Fdrt,1,0");
