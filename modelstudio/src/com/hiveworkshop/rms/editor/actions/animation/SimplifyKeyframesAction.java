...
public class SimplifyKeyframesAction implements UndoAction {
	...
	public void removeTransitionalKeyframes(AnimFlag<?> flag, int start, float valueDiff) {
...
//					System.out.println("Quat");
	...
	public void removeTransitionalKeyframesVec3(AnimFlag<Vec3> flag, int start, float valueDiff) {
...
//	}
	...
	public <T> void removeTransitionalKeyframes2(AnimFlag<T> flag, int start, float valueDiff) {
		Set<Entry<?>> entriesToRemoveSet = entriesToRemove.computeIfAbsent(flag, eS -> new HashSet<>());

		TreeMap<Integer, Entry<T>> entryMap = flag.getEntryMap();
		int end = entryMap.lastEntry().getKey();
