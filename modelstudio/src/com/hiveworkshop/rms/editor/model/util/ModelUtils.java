...
public final class ModelUtils {
	...
	public static Mesh createPlane(final byte firstDimension, final byte secondDimension, final Vector3 facingVector,
	...
	public static Mesh createPlane(final byte firstDimension, final byte secondDimension, final Vector3 facingVector,
		...
		for (int y = 0; y < (numberOfSegmentsY + 1); y++) {
			...
			for (int x = 0; x < (numberOfSegmentsX + 1); x++) {
				final Vector3 normal = new Vector3(facingVector.x, facingVector.y, facingVector.z);
				...
				vertex.addTVertex(new Vector2(x * segmentWidthUV1, y * segmentWidthUV2));
				vertices.add(vertex);
				if (y > 0) {
					if (x > 0) {
	 ...
	 * @param min
	 */
	public static void createBox(final EditableModel model, final Vector3 max, final Vector3 min, final int segments) {
		...
		for (final GeosetVertex vertex : geoset.getVertices()) {
			vertex.addTVertex(new Vector2(0, 0));
	 ...
	 * @return
	 */
	public static Mesh createBox(final Vector3 max, final Vector3 min, final int lengthSegs, final int widthSegs,
	 ...
	 * @param min
	 */
	public static void createGroundPlane(final EditableModel model, final Vector3 max, final Vector3 min, final int segments) {
		final Geoset geoset = new Geoset();
		geoset.setMaterial(new Material(new Layer("None", new Bitmap("textures\\white.blp"))));
