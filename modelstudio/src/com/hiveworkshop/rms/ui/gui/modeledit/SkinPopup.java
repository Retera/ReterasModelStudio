...
public class SkinPopup extends JPanel {
	...
	private final Bone[] bones = new Bone[BONE_COUNT];
	private final short[] weights = new short[BONE_COUNT];
	private final IdObjectChooserButton[] buttons = new IdObjectChooserButton[BONE_COUNT];
	private final IntEditorJSpinner[] spinners = new IntEditorJSpinner[BONE_COUNT];
	...
	public SkinPopup(EditableModel model, SkinBone[] skinBones) {
		...
		topPanel.add(Button.create("Sort", e -> sortByWeight()), "");
		topPanel.add(Button.create("Merge Same", e -> mergeSameBone()), "");
		...
		for (int i = 0; i < BONE_COUNT; i++) {
			...
			add(spinners[i], "");
			add(Button.create("remove", e -> removeBone(index)), "skip 1, wrap");
			if (skinBones != null && i < skinBones.length) {
	...
	private void updateMissingWeightsLabel() {
		...
		String token = 0 <= totMissingWeight ? "+" : "";
