...
public class AnimatedPerspectiveViewport extends BetterAWTGLCanvas implements AnimatedRenderEnvironment, RenderResourceAllocator {
	...
	private void loadDefaultCameraFor(final ModelView modelView) {
		...
		Vec4 axisHeapY = new Vec4(0, 1, 0, (float) Math.toRadians(yangle));
		inverseCameraRotationYSpin.setFromAxisAngle(axisHeapY);
		Vec4 axisHeapX = new Vec4(0, 0, 1, (float) Math.toRadians(xangle));

		inverseCameraRotationZSpin.setFromAxisAngle(axisHeapX);
	...
	private void updateAnimatedNodes() {
	...
	public void paintGL(final boolean autoRepainting) {
		...
		try {
			...
			renderGeosets(geosets, formatVersion);
...
//			e.printStackTrace();
	...
	private void paintAndUpdate(boolean autoRepainting) throws LWJGLException {
		if (autoRepainting) {
			swapBuffers();
	...
	private void setStandardColors(GeosetAnim geosetAnim, float geosetAnimVisibility, ComPerspRenderEnv timeEnvironment, Layer layer) {
		setStandardColors2(geosetAnim, geosetAnimVisibility, this, layer);
	}

	public void setStandardColors2(GeosetAnim geosetAnim, float geosetAnimVisibility, AnimatedRenderEnvironment timeEnvironment, Layer layer) {
		if (timeEnvironment.getCurrentAnimation() != null) {
			...
			if (/* geo.getMaterial().isConstantColor() && */ geosetAnim != null) {
				Vec3 renderColor = geosetAnim.getRenderColor(this);
				if (renderColor != null) {
