...
//		add(new JLabel("Destination"),   "sgx half, growx, wrap");

		add(getButton(donBoneButtonText, this::chooseDonBone, donModel), "");
		add(getButton(recBoneButtonText, this::chooseRecBone, recModel), "");
//		add(getButton(donBoneButtonText, this::chooseDonBone, donModel), "growy 0, ");
//		add(getButton(recBoneButtonText, this::chooseRecBone, recModel), "growy 0, wrap");
		...
		importButton.addActionListener(e -> doImport(chosenDonBone, chosenRecBone, boneChainDepth));
		add(importButton, "");
	}

	private void doImport(Bone donBone, Bone recBone, int boneChainDepth) {
		if (donBone != null && recBone != null) {
//			Map<IdObject, IdObject> chainMap = getChainMap(donBone, recBone, boneChainDepth); // receiving bones to donating bones
			Map<IdObject, IdObject> chainMap = getChainMap(donBone, donModel, recBone, recModel, boneChainDepth, false); // receiving bones to donating bones
