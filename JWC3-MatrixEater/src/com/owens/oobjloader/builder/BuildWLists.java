...
//    }
	...
	public void newMtl(final String name) {
        ...
        currentMaterialBeingParsed.dFactor = 1.0; // NOTE TODO MatrixEater here - default StaticAlpha on layer is 1.0
    ...
    /**
     * The code for this part looks ridiculous, but the point is that
     * we want to make sure when we convert to MDL that regardless of how
     * data was stored in the OBJ, the MDL will have the existence of two vertices
     * at the same world position only when they have different normals
     * or different texture coordinates.
     *
     ...
     * same key and therefore linking to the same physical vertex ID inside the MDL.
     *
     * @author Eric "Retera"
     *
     */
    private final class VertexKey {
    	...
    	public VertexKey(final VertexGeometric vertex, final VertexNormal normal, final VertexTexture uv) {
    		...
    		if( normal != null ) {
    	        ...
    	        //log.log(INFO, "@TODO: Got mesh without normal, should later add normal generation");
    		}
    		if( uv != null ) {
        		uvU = uv.u;
        		uvV = 1.0f-uv.v;
    ...
    public MDL createMDL() {
    	...
    	for(final Map.Entry<String, ArrayList<Face>> entry: groups.entrySet()) {
    		...
    		//final Map<VertexKey,GeosetVertex> builderVertexToMdlVertex = new HashMap<VertexKey,GeosetVertex>();
    		//Map
		...
		if( allLessThan2 ) {
			...
			final int option = JOptionPane.showOptionDialog(null,  "This model might be a WoW model, or peculiarly small. Would you like to increase its size?","WoW Scaling",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
			//final int result = JOptionPane.showConfirmDialog(null, "This model might be a WoW model, or peculiarly small. Would you like to increase its size?","WoW Scaling",JOptionPane.YES_NO_OPTION);
		...
		for(final com.hiveworkshop.wc3.mdl.Material material: mdl.getMaterials()) {
			for(final Layer layer: material.getLayers()) {
	    		...
	    		if( name.toLowerCase().endsWith(".png") || name.toLowerCase().endsWith(".tga") || name.toLowerCase().endsWith(".jpg") || name.toLowerCase().endsWith(".bmp") ) {
	    			hasPNGs = true;
	    		}
			}
		}
		...
		if( hasPNGs ) {
			final int result = JOptionPane.showConfirmDialog(null, "This OBJ model contains references to non-BLP files in its materials. Automatically create corresponding BLP files?\n\nIf you choose YES, the MDL format of this OBJ will also be generated to support Matrix Eater 3D viewing.","Convert Textures to BLPs",JOptionPane.YES_NO_OPTION);
		...
		final File mdlFile = new File(objFolder.getPath() + "/" + objFile.getName().substring(0,objFile.getName().length()-4) + ".mdl");
	    		...
	    		if( name.toLowerCase().endsWith(".png") || name.toLowerCase().endsWith(".tga") || name.toLowerCase().endsWith(".jpg") || name.toLowerCase().endsWith(".bmp") ) {
	    			if( userWantsSwapToBLP ) {
	    				try {
							...
							if( imageData == null ) {
								throw new RuntimeException("Java/MatrixEater failed to read image data: " + imageFilePNG.getAbsolutePath());
							}
							final File imageFileBLP = new File(objFolder.getPath() + "/" + name.substring(0, name.lastIndexOf('.')) + ".blp"); //name.replace(".png", ".blp").replace(".PNG", ".BLP"));
							BlpFile.writeJpgBLP(imageData, imageFileBLP, true, 0.90f);
//							BlpFile.writePalettedBLP(imageData, imageFileBLP, true, true, false);
	...
	private void convertMesh(final MDL mdl, final Set<Face> processedFaces,
			final String groupName, final Map<Material, Subgroup> materialToSubgroup,
			final ArrayList<Face> faceList) {
...
//	    				if( gv == null ) {
//	    					builderVertexToMdlVertex.put(key, gv = key.createVertex());
//	    				}
//	    				geoVertices[i] = gv;
		...
		for(final Map.Entry<Material,Subgroup> subgroup: materialToSubgroup.entrySet()) {
			...
			final List<VertexKey> vertexKeys = subgroup.getValue().getVertexKeys();//new ArrayList<VertexKey>();
			...
			for(final VertexKey key: vertexKeys) {
	    		...
	    		if( createdTVertex.getX() > 1.0 || createdTVertex.getX() < 0 || createdTVertex.getY() > 1.0 || createdTVertex.getY() < 0 ) {
...
//			10. Casts shadows onto invisible surfaces
		...
		if( colorOn ) {
			...
			if( color != null ) {
				geo.forceGetGeosetAnim().setStaticColor(new Vertex(color.bz, color.gy, color.rx));// pretty sure geoset anim is BGR off the top my head
			}
		}
		return mdlMaterial;
	}
