...
//	}
	...
	public void updateBoxes()
	{
		if( animFile != null )
		{
			final DefaultComboBoxModel<Animation> model = new DefaultComboBoxModel<Animation>();
			...
			new Thread(new Runnable() {
				...
				public void run() {
		            ...
		            while(importPanel.getParentFrame().isVisible() &&  (!importPanel.importStarted() || importPanel.importEnded()) )
		            {
		            	...
		            	try { Thread.sleep(1); } catch (final Exception e) {ExceptionPopup.display("MatrixEater detected error with Java's wait function",e); }
		            }
...
//                    }
                    ...
                    if( importPanel.importStarted() ){
                        ...
                        if( importPanel.importSuccessful() )
                        {
            				final ImportPanel importPanel2 = new ImportPanel(sourceFile,MDL.read(sourceFile.getFile()),show);
...
//    	                    }
	...
	public static void main(final String[] args) {
		...
		final AnimationTransfer transfer = new AnimationTransfer();
