...
public class RemoveGeometryPanel extends JPanel implements ActionListener {
	...
	public RemoveGeometryPanel(final RemovePanel parent, final Project project) {
		...
		for( final Attachment attachment: project.model.getMDL().sortedIdObjects(Attachment.class) ) {
			...
			text = "\"" + text + "\"";
			if( !text.toLowerCase().startsWith("sprite")
					&& !text.toLowerCase().startsWith("origin")
...
//		glow = new JButton("Glow", new ImageIcon(new ImageIcon(RemoveGeometryPanel.class.getResource("BTNTeamGlow00.png")).getImage().getScaledInstance(48, 48, Image.SCALE_SMOOTH)));
