	...
	public static void scale(final MDL mdl, final double x, final double y, final double z) {
		...
		for (final Geoset geoset : mdl.getGeosets()) {
			for (final Vertex vertex : geoset.getVertices()) {
				vertex.scale(0, 0, 0, x, y, z);
			}
			for (final Animation anim : geoset.getAnims()) {
				scale(x, y, z, anim.getExtents());
			}
		}
		...
		for (final Camera camera : mdl.getCameras()) {
			camera.getPosition().scale(0, 0, 0, x, y, z);
			camera.getTargetPosition().scale(0, 0, 0, x, y, z);
		}
		...
		scale(x, y, z, mdl.getExtents());
		for (final Animation anim : mdl.getAnims()) {
			scale(x, y, z, anim.getExtents());
		}
	}

	private static void scale(final double x, final double y, final double z, final ExtLog extents) {
		if (extents == null) {
			return;
		}
		if (extents.getMaximumExtent() != null) {
			extents.getMaximumExtent().scale(0, 0, 0, x, y, z);
		}
		if (extents.getMinimumExtent() != null) {
			extents.getMinimumExtent().scale(0, 0, 0, x, y, z);
		}
	}
