...
public class ParticleEmitter2Chunk {
	...
	public class ParticleEmitter2 {
		...
		public ParticleEmitter2(com.hiveworkshop.wc3.mdl.ParticleEmitter2 mdlEmitter) {
...
//			ADDALPHA("AlphaKey");
			...
			for( String flag: mdlEmitter.getFlags() ) {
				switch (flag) {
				case "Head":
					headOrTail = 0;
					break;
			...
			for( int i = 0; i < mdlEmitter.getSegmentColorCount(); i++ ) {
				Vertex color = mdlEmitter.getSegmentColor(i);
				segmentColor[index++] = (float)color.getZ();
				segmentColor[index++] = (float)color.getY();
				segmentColor[index++] = (float)color.getX();
			}
			segmentAlpha = new byte[]{
					(byte)mdlEmitter.getAlpha().getX(),
					(byte)mdlEmitter.getAlpha().getY(),
			...
			for( AnimFlag af: mdlEmitter.getAnimFlags() ) {
				if( af.getName().equals("Visibility") ) {
					...
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2Visibility.ScalingTrack mdxEntry = particleEmitter2Visibility.new ScalingTrack();
					...
					particleEmitter2EmissionRate.scalingTrack = new ParticleEmitter2EmissionRate.ScalingTrack[af.size()];
					boolean hasTans = af.tans();
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2EmissionRate.ScalingTrack mdxEntry = particleEmitter2EmissionRate.new ScalingTrack();
					...
					boolean hasTans = af.tans();
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2Latitude.ScalingTrack mdxEntry = particleEmitter2Latitude.new ScalingTrack();
					...
					boolean hasTans = af.tans();
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2Length.ScalingTrack mdxEntry = particleEmitter2Length.new ScalingTrack();
					...
					boolean hasTans = af.tans();
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2Width.ScalingTrack mdxEntry = particleEmitter2Width.new ScalingTrack();
					...
					boolean hasTans = af.tans();
					for( int i = 0; i < af.size(); i++ ) {
						ParticleEmitter2Speed.ScalingTrack mdxEntry = particleEmitter2Speed.new ScalingTrack();
