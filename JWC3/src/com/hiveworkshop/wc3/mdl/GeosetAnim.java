...
public class GeosetAnim implements VisibilitySource, Named
{
     ...
     */
    @Deprecated
	public GeosetAnim(int geosetId) {
    	this.geosetId = geosetId;
    }
    public GeosetAnim(GeosetAnimationChunk.GeosetAnimation geosetAnim) {
		...
		if ( (geosetAnim.flags & 2) == 2 ) {
			if( geosetAnim.geosetColor == null ) {
				Vertex coloring = new Vertex(geosetAnim.color);
...
//             MDLReader.reset(mdl);
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse GeosetAnim: Missing or unrecognized open statement.");
        }
        return null;
    }
	...
	public void addAnimFlag(AnimFlag af) {
		animFlags.add(af);
	}
	...
	public void setStaticColor(Vertex staticColor) {
	 ...
	 */
	@Deprecated
	public void setGeosetId(int geosetId) {
		this.geosetId = geosetId;
	}
