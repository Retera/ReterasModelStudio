...
public class AnimFlag
{
    ...
    public AnimFlag(final TextureRotation textureData) {
		...
		final boolean tans = textureData.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???;
		for( final TextureRotation.TranslationTrack track: textureData.translationTrack ) {
			if( tans ) {
				addEntry(track.time, new QuaternionRotation(track.rotation), new QuaternionRotation(track.inTan), new QuaternionRotation(track.outTan));
			} else {
    ...
    public AnimFlag(final GeosetAlpha geosetAlpha) {
		...
		final boolean tans = geosetAlpha.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public AnimFlag(final GeosetColor geosetColor) {
		...
		final boolean tans = geosetColor.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public AnimFlag(final GeosetTranslation geosetTranslation) {
		...
		final boolean tans = geosetTranslation.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public AnimFlag(final GeosetRotation geosetData) {
		...
		final boolean tans = geosetData.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public AnimFlag(final LightVisibility trackData) {
		...
		final boolean tans = trackData.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public AnimFlag(final CameraPositionTranslation translation) {
		...
		final boolean tans = translation.interpolationType > 1; // NOTE: autoreplaced from a > 0 check, Linear shouldn't have 'tans'???
    ...
    public void generateTypeId() {
        ...
        else if( title.equals("TextureID") )//aflg.title.equals("Visibility") ||  -- 100.088% visible in UndeadCampaign3D OutTans! Go look!
        {
            typeid = 5;
        }
	...
	/**
	 * This class is a small shell of an example for how my "AnimFlag" class should've been implemented.
	 * It's currently only used for the {@link AnimFlag#getEntry(int)} function.
	 *
	 * @author Eric
	 *
	 */
    ...
    public static AnimFlag parseText(final String [] line)
    {
        ...
        {
            JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Unable to parse \""+aflg.title+"\": Missing or unrecognized open statement.");
        }
        aflg.typeid = typeid;
        for( int i = 1; i < line.length; i++ )
        {
            ...
            else if( line[i].contains("GlobalSeqId") )
            {
                ...
                {
                    JOptionPane.showMessageDialog(MDLReader.getDefaultContainer(),"Error while parsing "+aflg.title+": More than one Global Sequence Id is present in the same "+aflg.title+"!");
                }
            }
            else
            {
...
//                         break;
    ...
    public AnimFlag getMostVisible(final AnimFlag partner)
    {
        if( partner != null )
        {
            ...
            {
                final ArrayList<Integer> atimes = new ArrayList<Integer>(times);
                final ArrayList<Integer> btimes = new ArrayList<Integer>(partner.times);
...
//                         bvalues.remove(partVal);
    ...
    /**
     * Copies time track data from a certain interval into a different, new interval. The
     * AnimFlag source of the data to copy cannot be same AnimFlag into which the
    ...
    public void copyFrom( final AnimFlag source, final int sourceStart, final int sourceEnd, final int newStart, final int newEnd )
    {
        ...
        if( tans && tags.contains("Linear") ) {
        	final int x = JOptionPane.showConfirmDialog(null, "ERROR! A source was found to have Linear and Nonlinear motion simultaneously. Does the following have non-zero data? "+source.inTans, "Help This Program!", JOptionPane.YES_NO_OPTION);
