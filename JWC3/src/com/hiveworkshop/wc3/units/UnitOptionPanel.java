...
public class UnitOptionPanel extends JPanel implements ActionListener {
	...
	DataTable unitData = DataTable.get();
...
//		playerBox.setMaximumSize(new Dimension(10000,25));
	...
	class RaceData {
		...
		List<Element> heroes = new ArrayList<Element>();
		List<Element> buildings = new ArrayList<Element>();
		List<Element> buildingsUprooted = new ArrayList<Element>();
		List<Element> special = new ArrayList<Element>();
		
		void sort() {
			Comparator<Element> unitComp = new ElementComparator();
	...
	public void sortRaces() {
		if ( sortedRaces == null ) {
			...
			Element root = unitData.get("Aroo");
			Element rootAncientProtector = unitData.get("Aro2");
			Element rootAncients = unitData.get("Aro1");
			for( String str: unitData.keySet() ) {
				...
				Element unit = unitData.get(str);
				String raceKey = "neutrals";
				List<GameObject> abilities = unit.abilities();
...
//				}
		...
		good = false;
		for( Element unit: data.buildings ) {
			if( isNeutral && !unit.getField("tilesets").contains(tileset) && !unit.getField("tilesets").contains("*") && !unit.getField("tilesets").contains("_") ) {
				continue;
			}
			if( checkLevel && unit.getFieldValue("level") != levelBox.getSelectedIndex() - 1) {
...
//				}
		...
		good = false;
		for( Element unit: data.buildingsUprooted ) {
			if( isNeutral && !unit.getField("tilesets").contains(tileset) && !unit.getField("tilesets").contains("*") && !unit.getField("tilesets").contains("_") ) {
//				System.err.println(unit.getField("Name") + " failed for tilset");
				continue;
			}
